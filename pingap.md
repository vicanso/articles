...
Cloudflare开源了其基于rust构建的反向代理框架`pingora`，已经在Cloudflare的云服务上运行多年，每秒支撑起超过4000万个互联网请求(需要注意，此处并不是性能指标)。以往使用golang开发了类似varnish的软件pike，本次则准备开发类似nginx的软件[pingap](https://github.com/vicanso/pingap)。
...
`pingap`整体的设计比较简单，基于`host`与`path`选择匹配的`location`，之后将请求转发至对应的`upstream`，而`pingora`的处理流程可以查看官方说明的[phase_chart](https://github.com/cloudflare/pingora/blob/main/docs/user_guide/phase_chart.md)，下面我们从零开始构建`pingap`。
...
- `pingap`中对于upstream有对应的health check，支持`http`与`tcp`的形式，若未指定则默认以`tcp`的形式检测端口是否可连接，因此若所有节点均检测不通过时，则返回`None`。
- `pingora`与upstream的连接是可复用的，若不想复用则设置`idle_timeout`为0，默认`None`表示过无期清除时长。而`upstream_keepalive_pool_size`默认为128的连接池大小，但是`pingora`暂未提供直接设置该值的方式，只能通过`yaml`配置加载，一般也不需要调整。
...
path匹配的权重规则如下`全等匹配` > `前缀匹配` > `正则匹配`，相同类型的path匹配，若有配置host则权重较高，而相同权重的则按配置长度排序，较长的配置权重较高。

location的path改写支持正则的方式改写，如配置`rewrite: Some("^/users/(.*)$ /$1".to_string())`，则针对以`/users`开头的请求将其删除。请求头与响应头的处理则很简单，仅简单的请求发送至`upstream`时设置请求头，在响应到`downupstream`则设置响应头。

## Server

Server关联了其对应的所有location，在接收到`downstream`的请求后，根据权重一一匹配其所有location，选择到符合的则请求对应的`upstream`将响应数据，下面先来介绍一下Server的定义：
...
`pingora`默认实现了`ProxyHttp`大部分实现，完整的处理流程参数[phase_chart](https://github.com/cloudflare/pingora/blob/main/docs/user_guide/phase_chart.md)，因为Server部分针对需求实现相应的实现，下面是简化的说明：
